...
best way to transfer them to JASMIN?

1. `scp -r data jdoe@login.jasmin.ac.uk:~/`
2. `rsync -ra data jdoe@login.jasmin.ac.uk:~/`
3. `rsync -raz data jdoe@login.jasmin.ac.uk:~/`
4. `tar -cvf data.tar data;`

   `rsync -raz data.tar jdoe@login.jasmin.ac.uk:~/`
5. `tar -cvzf data.tar.gz data;`

   `rsync -ra data.tar.gz jdoe@login.jasmin.ac.uk:~/`
