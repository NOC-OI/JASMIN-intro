...
```{r, echo=FALSE, results='asis'}
snippets('scheduler/basic-job-status.Rmd')
```
...
```{r, echo=FALSE, results='asis'}
snippets('scheduler/basic-job-status-name-change.Rmd')
```
...
```{r, echo=FALSE, results='asis'}
snippets('scheduler/cancel-job.Rmd')
```
...
```bash
`r config$remote$prompt` `r config$sched$del` 13
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$submit$options` hostname
```
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$submit$options` -n 2 echo "This job will use 2 CPUs."
```
...
```bash
`r config$remote$prompt` `r config$sched$interactive` `r config$sched$flag$interactive` --pty bash
```
...
the `ssh` command, e.g., `ssh -Y -J `r config$remote$user`@`r config$remote$login` `r config$remote$user`@`r config$remote$launch``.
