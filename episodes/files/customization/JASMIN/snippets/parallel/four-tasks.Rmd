...
`r config$sched$comment` `r config$sched$flag$account` `r config$sched$queue$account`
`r config$sched$comment` `r config$sched$flag$qos` `r config$sched$queue$qos`
`r config$sched$comment` -N 1
`r config$sched$comment` -n 4

# Load the computing environment we need
# (mpi4py and numpy are in `r config$remote$modules$python`)
